...
use crate::model::notification::queued::QueuedNotification;
...
impl Task for NotificationSend {
    ...
    async fn perform(&self) -> Result<Self::Output, BoxError> {
        let (Some(role), Some(subject), Some(body)) = (
            ...
            tracing::warn!(
                "Queue: Notification skipped due to missing required fields (role, subject, or body)"
            );
            return Ok(());
        };
        let emails = RoleRecipients::new(self.pool.clone(), role.clone())
            .items()
            .await?;
